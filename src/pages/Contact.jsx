...
const Contact = () => {
  ...
  const replaceSchoolName = (text) => typeof text === 'string' ? text.replace('{schoolName}', branding.schoolName) : text;
      ...
      <section className="py-20 px-4 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
          ...
          <div className="space-y-8">
            <h2 className="text-3xl lg:text-4xl font-bold text-slate-800 tracking-tight">{t('contactGetInTouch')}</h2>
            ...
            <div className="space-y-6 pt-4">
              <div className="flex items-start gap-5 p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:border-school-blue/30 transition-colors">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 text-lg mb-1">{t('contactLocationTitle')}</h4>
              ...
              <div className="flex items-start gap-5 p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:border-school-green/30 transition-colors">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 text-lg mb-1">{t('contactPhoneTitle')}</h4>
              ...
              <div className="flex items-start gap-5 p-6 bg-white rounded-2xl shadow-sm border border-slate-100 hover:border-amber-500/30 transition-colors">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 text-lg mb-1">{t('contactEmailTitle')}</h4>
              ...
              {isSubmitted ? (
                <div className="py-24 text-center space-y-6 animate-in zoom-in duration-500">
                  ...
                  <p className="text-slate-500 max-w-sm mx-auto text-lg leading-relaxed">
                    {replaceSchoolName(t('contactThankYou'))}
                  </p>
                  <div className="pt-6">
                      <button onClick={() => setIsSubmitted(false)} className="text-school-blue font-bold hover:underline">{t('contactSendAnother')}</button>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  <h3 className="text-2xl font-bold text-slate-800 mb-8">{t('contactSendMessageTitle')}</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">{t('contactYourName')}</label>
                      <input 
                        type="text" 
                        name="name"
                        value={formData.name}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">{t('contactEmailAddressLabel')}</label>
                      <input 
                        type="email" 
                        name="email"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">{t('contactSubjectLabel')}</label>
                    <input 
                      type="text" 
                      name="subject"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">{t('contactYourMessageLabel')}</label>
                    <textarea 
                      name="message"
                      rows="5"
                  ...
                  <button type="submit" className="w-full btn-primary py-4 text-lg flex items-center justify-center gap-2 mt-4 group">
                    {t('contactSendMessageTitle')} <Send size={20} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                  </button>
                </form>
              )}
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 mb-20">
        <div className="w-full bg-slate-900 rounded-[2rem] overflow-hidden relative shadow-2xl border-t-8 border-school-blue/20">
          <div className="p-8 bg-white/5 backdrop-blur-md flex justify-between items-center border-b border-white/10 relative z-10">
             <div>
               <p className="font-black text-white text-2xl tracking-tighter italic uppercase">{t('contactSchoolLocation')}</p>
             ...
             >
              {t('contactOpenMap')} <MapPin size={18} />
             </a>
          </div>
