...
const Footer = () => {
  ...
  const handleAdminLogin = async () => {
    try {
      ...
      if (!auth) {
        alert("Firebase is not configured yet. Please set VITE_FIREBASE_API_KEY in .env.local");
        return;
      }
      const result = await signInWithPopup(auth, googleProvider);
      const email = result.user.email;
      if (email === 'yvensiyonkuru2022@gmail.com' || email === 'yvesniyonkuru2022@gmail.com') {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 border-b border-white/10 pb-12">
          ...
          <div className="space-y-4 col-span-1 md:col-span-1">
            <SchoolBrand name={branding.schoolName} as="h3" className="text-xl font-bold mb-4 block" />
